...
interface Conversation {
  ...
  created_at: string;
  customer?: { id: string; name: string; phone: string; avatar_url: string | null };
...
export default function Inbox() {
  ...
  async function fetchConversations() {
    const { data } = await supabase.from('conversations').select('*, customer:customers(*)').order('last_message_at', { ascending: false });
    if (!data) return;

    const convIds = data.map(c => c.id);
    const [tagsRes, profilesRes] = await Promise.all([
      convIds.length > 0 ? supabase.from('conversation_tags').select('conversation_id, tag:tags(*)').in('conversation_id', convIds) : { data: [] },
  ...
  async function handleSend() {
    if (!selectedConv || !user || !newMessage.trim()) return;
  ...
  async function handleAssign(convId: string) {
    if (!user) return;
    await supabase.from('conversations').update({ assigned_user_id: user.id, status: 'open' }).eq('id', convId);
  ...
  return (
    <AppLayout>
      <div className="flex h-full">
        {/* Conversation list */}
        <div className={cn(
          "w-full md:w-80 lg:w-96 border-r border-border flex flex-col shrink-0 bg-card/30",
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {filtered.map(conv => (
              <button
                ...
                className={cn(
                  'w-full flex items-center gap-3 p-3 border-b border-border/50 transition-colors text-left',
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <div className={cn(
                      ...
                      conv.status === 'open' ? 'bg-primary' : conv.status === 'unassigned' ? 'bg-warning' : 'bg-muted-foreground/30'
                    )} />
                  </div>
                </div>
              </button>
            ...
            <>
              <div className="h-14 border-b border-border flex items-center px-4 gap-3 shrink-0">
                ...
                <div className="flex items-center gap-1">
                  {selected.tags?.map(tag => (
                    <Badge key={tag.id} variant="outline" className="text-[10px] h-5" style={{ borderColor: tag.color, color: tag.color }}>{tag.name}</Badge>
                  ))}
                  {selected.status !== 'closed' && (
                    <>
                      <Button size="sm" variant="ghost" className="h-8 text-xs" onClick={() => handleAssign(selected.id)}>
                        <UserPlus className="w-3.5 h-3.5 mr-1" /> Atribuir
                      </Button>
                      <Button size="sm" variant="ghost" className="h-8 text-xs text-destructive" onClick={() => handleClose(selected.id)}>
              ...
              <div className="p-3 border-t border-border">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <Button size="icon" className="shrink-0 h-9 w-9" onClick={handleSend} disabled={!newMessage.trim()}>
          ...
          <div className="hidden xl:flex w-72 border-l border-border flex-col p-4 shrink-0 bg-card/30">
            <div className="text-center mb-6">
              ...
              <p className="text-xs text-muted-foreground flex items-center justify-center gap-1 mt-1">
                <Phone className="w-3 h-3" /> {selected.customer?.phone}
              </p>
            </div>
            <div className="space-y-3 text-xs">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-foreground">{selected.assigned_profile?.name || 'Ninguém'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Criado em</span>
                <span className="text-foreground">{format(new Date(selected.created_at), 'dd/MM/yy')}</span>
              </div>
              {selected.tags && selected.tags.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {selected.tags.map(tag => (
                      <Badge key={tag.id} variant="outline" className="text-[10px]" style={{ borderColor: tag.color, color: tag.color }}>{tag.name}</Badge>
                    ))}
                  </div>
                </div>
              )}
